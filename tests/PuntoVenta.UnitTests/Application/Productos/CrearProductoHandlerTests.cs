...
public class CrearProductoHandlerTests
{
    ...
    public async Task Handle_DebeCrearProducto_CuandoDatosMinimosValidos()
    {
        ...
        var handler = new CrearProductoHandler(usuarioActual, repo, categoriaRepo, new FakeMovimientoStockRepository(), new FakeFechaActual(), permisoCache);
    ...
    public async Task Handle_DebeCargarStockInicial_YRegistrarMovimiento_CuandoExistenciaInicialPositiva()
    {
        ...
        var handler = new CrearProductoHandler(usuarioActual, repo, categoriaRepo, movimientoRepo, new FakeFechaActual(), permisoCache);

        var command = new CrearProductoCommand("P010", "Producto", TipoItem.Bien, 1000m, ExistenciaInicial: 25m);
        var resultado = await handler.Handle(command, CancellationToken.None);
