...
public sealed class ObtenerReciboPagoApartadoPdfEndpoint(IMediator mediator) : EndpointWithoutRequest
{
    private readonly IMediator _mediator = mediator;

    public override void Configure()
    {
        Get("/ventas/apartados/{id:guid}/abonos/{pagoId:guid}/pdf");
        Tags("Ventas");
        Options(b => b.RequireAuthorization(PermisosRegistrar.Claves.FacturacionVer));
        Summary(s => s.Summary = "Obtener PDF de recibo de abono");
    }
